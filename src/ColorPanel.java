...
public class ColorPanel extends JPanel implements MouseListener,ActionListener{//調色盤class
	...
	private	JPanel jPanel_color1[]=new JPanel[30];
	private	JPanel jPanel_color2[]=new JPanel[30];
	private	ImageIcon special_color[]= new ImageIcon[4];
	private BufferedImage bufImg = new BufferedImage(12 ,12,BufferedImage
			.TYPE_3BYTE_BGR) ,bufImg2 = new BufferedImage(12 ,12
	...
	public ColorPanel(Variable var,Painter paint){//產生版面//
		this.variable = var;
		this.painter = paint;
		...
		special_color[0] = new ImageIcon( getClass().getResource("img/icon1.gif") );
		special_color[1] = new ImageIcon( getClass().getResource("img/icon2.gif") );
		special_color[2] = new ImageIcon( getClass().getResource("img/icon3.gif") );
		special_color[3] = new ImageIcon( getClass().getResource("img/icon4.gif") );
		
		this.setLayout(null);
		variable.color_border=new Color(0,0,0);
		variable.color_inside=null;
		...
		for(int i=0;i<jPanel_color1.length;i++){
			...
			jPanel_color1[i].setBorder(BorderFactory.createEtchedBorder
					(BevelBorder.RAISED));
		}
		...
		jDialog = new JDialog(painter, "請選擇兩種顏色做漸層", true);
	...
	public void mousePressed( MouseEvent e ){
		...
		if(e.getX()>=5 && e.getX()<=20 && e.getY()>=5 && e.getY()<=20){
			...
			variable.color_border = JColorChooser.showDialog( painter
					, "請選擇邊線顏色"
					, (Color)variable.color_border );
			...
			variable.color_inside = JColorChooser.showDialog( painter
					, "請選擇填充顏色"
					, (Color)variable.color_inside );
		...
		if(choose==28){//填充無顏色
			...
			g2d.setPaint( new GradientPaint( 0, 0, center.G_color_left
					, 12, 12, center.G_color_right, true ) );
			g2d.fill( new Rectangle2D.Double( 0, 0, 12, 12 ) );
			repaint();
			
			if(e.getButton()==1)
				variable.color_border=new GradientPaint( 0, 0, center.G_color_left
						, 12, 12, center.G_color_right, true );
			else
				variable.color_inside=new GradientPaint( 0, 0, center.G_color_left
						, 12, 12, center.G_color_right, true );
		}
		/*else if(choose==30){//填充圖案
			...
			BufferedImage bufferedImage = new BufferedImage(icon.getIconWidth()
					,icon.getIconHeight(),BufferedImage.TYPE_3BYTE_BGR);//創一張新的BufferedImage，為了要讀取讀進來的圖片長寬，以免有空白
			bufferedImage.createGraphics().drawImage(icon.getImage(),0,0,this);//把icon畫到BufferedImage上
			repaint();//重繪螢幕
			
			if(e.getButton()==1)//判斷邊線顏色或內部填滿色
				variable.color_border=new TexturePaint(bufferedImage
						, new Rectangle( icon.getIconWidth(), icon.getIconHeight() ) );//把這張BufferedImage設成TexturePaint來填滿
			else
				variable.color_inside=new TexturePaint(bufferedImage
			...
			FontColor = JColorChooser.showDialog( painter
					, "請選擇一個顏色當文字顏色", FontColor );//讓使用者選擇顏色
			
			g2d.drawImage(special_color[3].getImage(), 0, 0,this);//把調色盤左方換成『字』
			
			BufferedImage bufferedImage = new BufferedImage(
					variable.draw_panel_width
					,variable.draw_panel_height,BufferedImage.TYPE_3BYTE_BGR);//創一張新的BufferedImage
			...
			if(e.getButton()==1)//判斷邊線顏色或內部填滿色
				variable.color_border=new TexturePaint(bufferedImage, new Rectangle(sw,sh) );//把這張BufferedImage設成TexturePaint來填滿
			else
				variable.color_inside=new TexturePaint(bufferedImage, new Rectangle(sw,sh) );
		}*/
		else{//填充一般色
			...
			if(e.getButton()==1)
				variable.color_border=new Color(rgb[0][choose],rgb[1][choose],rgb[2][choose]);
			else
				variable.color_inside=new Color(rgb[0][choose],rgb[1][choose],rgb[2][choose]);
		}
	}
